...
async function sheetsReq(method, path, body, token) {
  return new Promise((resolve, reject) => {
    ...
    const req = https.request({
      ...
      res.on('end',()=>{
        try {
          ...
          resolve(txt.replace(/```json|```/g,'').trim());
        } catch(e){reject(e);}
      });
    });
    req.on('error',reject); req.write(body); req.end();
...
app.post('/api/servicos', async (req,res) => {
  try { await sheetWrite('Servicos', req.body); res.json({ok:true}); }
  catch(e){ res.status(500).json({error:e.message}); }
});

// Parse text with AI → create task + optional financial
app.post('/api/parse-text', async (req,res) => {
  try {
    const { text } = req.body;
    const parsed = await parseTaskPrompt(text);
    const id = Date.now().toString();
    const now = new Date().toLocaleDateString('pt-BR');

    // Build task
    const task = {
      id, texto: parsed.texto||text,
...
app.post('/api/parse-audio', upload.single('audio'), async (req,res) => {
  try {
    ...
    const raw = await geminiAudio(audioBase64, mimeType);
